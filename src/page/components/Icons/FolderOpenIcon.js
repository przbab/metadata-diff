export class FolderOpenIcon extends HTMLElement {
    constructor() {
        super();

        this.root = this.attachShadow({ mode: 'closed' });
        this.root.innerHTML = `
<style>
    svg {
        fill: var(--folder-icon-color);
        width: 16px;
        height: 16px;
    }
</style>
<svg aria-label="File open" aria-hidden="true" viewBox="0 0 16 16" >
    <path d="M.513 1.513A1.75 1.75 0 0 1 1.75 1h3.5c.55 0 1.07.26 1.4.7l.9 1.2a.25.25 0 0 0 .2.1H13a1 1 0 0 1 1 1v.5H2.75a.75.75 0 0 0 0 1.5h11.978a1 1 0 0 1 .994 1.117L15 13.25A1.75 1.75 0 0 1 13.25 15H1.75A1.75 1.75 0 0 1 0 13.25V2.75c0-.464.184-.91.513-1.237Z"></path>
</svg>`;
    }
}

window.customElements.define('folder-open-icon', FolderOpenIcon);
